...
const root=path.resolve(__dirname,'../public'),context={window:{}};
...
function challenges(items){return items.map(c=>{
 ...
 if(c.kind==='argument')text+='**Supported connections:**\n\n'+c.answer.map(([a,b])=>'- '+c.items[a]+' → '+c.items[b]).join('\n')+'\n\n';
...
for(const unit of data.units){const lessons=data.lessons.filter(l=>l.unit===unit.id);if(!lessons.length)continue;
 ...
 for(const extra of l.extraReadings||[]){const[b,c,s,e]=extra.ref;text+=`\n### Additional reading: ${extra.reading} · Berean Standard Bible\n\n`+corpus.filter(r=>+r[0]===b&&+r[1]===c&&+r[2]>=s&&+r[2]<=e).map(r=>`**${r[2]}** ${r.slice(3).join('\t')}`).join('\n\n')+'\n';}
