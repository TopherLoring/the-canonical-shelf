...
assert.ok(sw.includes('canon-v5-experience-6'),'service-worker cache version must target the current application');
...
for(const asset of ['./v5-shell.css','./v5-shell.js','./v5-pages.css','./v5-pages.js','./v5-mobile-bible.css','./v5-book-study-data.js','./v5-mobile-bible.js','./v5-learning-experience.css','./v5-learning-experience.js','./v5-inline-scripture.js','./v5-guided-tour.js','./v5-onboarding.css','./v5-bible-continuity.js','./v5-progress-adapter.js','./v5-blueprint-r3.css'])assert.ok(sw.includes(asset),`application asset ${asset} must be precached`);
...
assert.ok(sw.includes('./v4-shell-bridge.js'),'stable integration bridge must remain available offline beneath the current shell');
...
console.log(`PASS offline cache: preview + ${assets.length} local runtime assets + current onboarding/learning/Bible experience are precached`);
