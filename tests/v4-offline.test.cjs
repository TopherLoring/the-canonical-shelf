...
assert.ok(sw.includes('canon-v5-experience-3'),'service-worker cache version must target the current application');
...
for(const asset of ['./v5-shell.css','./v5-shell.js','./v5-pages.css','./v5-pages.js','./v5-mobile-bible.css','./v5-book-study-data.js','./v5-mobile-bible.js'])assert.ok(sw.includes(asset),`application asset ${asset} must be precached`);
...
console.log(`PASS offline cache: preview + ${assets.length} local runtime assets + current mobile/Bible experience are precached`);
