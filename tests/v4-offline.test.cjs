...
assert.ok(sw.includes('canon-v5-experience-2'),'service-worker cache version must target the complete v5 application');
...
for(const asset of ['./v5-shell.css','./v5-shell.js','./v5-pages.css','./v5-pages.js'])assert.ok(sw.includes(asset),`v5 application asset ${asset} must be precached`);
...
console.log(`PASS offline cache: preview + ${assets.length} local runtime assets + stable runtime + complete v5 application are precached`);
