...
assert.ok(sw.includes('canon-v5-experience-1'),'service-worker cache version must target the v5 application shell');
...
for(const asset of ['./v5-shell.css','./v5-shell.js'])assert.ok(sw.includes(asset),`v5 shell asset ${asset} must be precached`);
...
console.log(`PASS offline cache: preview + ${assets.length} local runtime assets + v4 runtime + v5 shell are precached`);
