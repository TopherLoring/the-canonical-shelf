...
window.Foundations = (() => {
  ...
  function resetChallenge(){const c=D.lessons.find(l=>l.id===run.id).challenges[run.index];run.order=c.items.map((_,i)=>i);if(state.lessons[run.id].reviews%2)run.order.reverse();run.matches={};run.selected=[];run.stage=0;run.feedback='';run.correct=false;run.hint=false;}
  ...
  function game() {
    const l=D.lessons.find(l=>l.id===run.id),c=l.challenges[run.index];
    let board='';
    ...
    shell(`<p class="fd-kicker">${esc(l.title)} · ${run.review?'Return mission':'Understanding mission'} · ${run.index+1}/${l.challenges.length}</p><h2 tabindex="-1">${esc(c.title)}</h2><p>${esc(c.prompt)}</p><progress aria-label="Challenges solved" value="${run.solved}" max="${l.challenges.length}"></progress>${board}<div class="fd-actions">${c.kind!=='scenario'&&!run.correct?button('check','Test your construction'):''}${button('hint','Reveal a hint')}${button('lesson','Revisit the lesson',`data-id="${l.id}"`)}</div>${run.hint?`<p class="fd-feedback">${esc(c.hint)}</p>`:''}<div id="fd-feedback" role="status" aria-live="polite" tabindex="-1">${esc(run.feedback)}</div>${run.correct?button('next',run.index+1===l.challenges.length?'Complete mission':'Next challenge'):''}`);
  ...
  function act(action,data={}) {
    ...
    if(action==='lesson'){view={type:'lesson',id:data.id};run=null;}
    if(action==='start'){start(data.id);return;}
    if(run){const l=D.lessons.find(l=>l.id===run.id),c=l.challenges[run.index];
      ...
      if(action==='next'&&run.correct){if(run.index+1<l.challenges.length){run.index++;resetChallenge();}else {state.lessons[l.id]=award(state.lessons[l.id],Date.now(),run.review);persist();view={type:'done',id:l.id,review:run.review,early:run.wasPassed&&!run.review};run=null;}}
    }
    render();
  ...
  function render(target) {
    ...
    if(view.type==='done'){const l=D.lessons.find(l=>l.id===view.id);shell(`<h2 tabindex="-1">${view.early?'Practice complete':view.review?'Return mission complete':'Milestone earned'}</h2><p>You reconstructed and applied the ideas in ${esc(l.title)}.</p><p>${view.early?'Early practice keeps ideas fresh; your scheduled review date stays the same.':esc(status(l))}</p>${button('home','Back to learning map')}${button('practice','Choose another mission')}`);return;}
