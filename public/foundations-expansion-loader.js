/* Static-host loader for the Canonical Shelf curriculum.
   On the v4 redesign branch this builds the 70-lesson source, then mounts v4 Course + Topics into the full reader/search shell. */
(() => {
  'use strict';
  if (window.__CANON_EXPANSION_LOADER__) return;
  window.__CANON_EXPANSION_LOADER__ = true;

  const STYLES = ['v4-design-system.css','v4-integrated.css'];
  const FILES = [
    ...
    'v4-course-map.js',
    'v4-guided-visual-rules.js',
    'v4-curriculum-migration.js',
    'v4-visuals.js',
    'v4-games.js',
    'v4-guided-game-adapter.js',
    'v4-progress-migration.js',
    'v4-guided-shell.js',
    ...
    'v4-mastery-manifest.js',

    'topics-data.js',
    'topics-extended.js',
    'v4-topic-bridge.js',
    'topics.js',
    'v4-topics-enhance.js',

    'v4-app-integrated.js',
    'v4-shell-bridge.js'
  ];
  ...
  const loadStyle = src => new Promise((resolve,reject) => {
    ...
    el.rel='stylesheet';el.href=href;el.dataset.canonV4Style=src;
  ...
  async function boot(){
    ...
    try{
      for(const style of STYLES) await loadStyle(style);
      for(const file of FILES) await load(file);
    }catch(err){
      window.__CANON_EXPANSION_BOOTED__=false;
      console.error('Canonical Shelf v4 curriculum failed to load.',err);
    }
  }
