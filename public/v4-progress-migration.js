...
window.CanonV4Progress=(()=>{
  ...
  function completeMastery(id){state.mastery[id]??={done:false,attempts:0};state.mastery[id].done=true;state.mastery[id].attempts++;save();}
  ...
  return {get state(){return state},save,completeMastery,attemptMastery,completeLesson,setNote,recentTopic,exportState,importState};
})();
