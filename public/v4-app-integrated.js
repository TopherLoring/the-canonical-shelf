/* Unified Canonical Shelf v4 course: 70 guided lessons + 69 mastery modules across 23 units. */
window.CanonV4Integrated=(()=>{
  ...
  function progress(done,total,label='activities'){return `<div class="v4-progress"><small>${done}/${total}</small><div class="v4-progress__track"><div class="v4-progress__fill" style="width:${total?done/total*100:0}%"></div></div><small>${esc(label)}</small></div>`;}
  function map(){current={type:'map'};const all=overall();host.innerHTML=`<div class="v4-shell"><section class="v4-hero"><p class="v4-eyebrow">Canonical Shelf v4 · unified curriculum</p><h2>Learn the library, story, history, and theology as one connected course.</h2><p>23 units · 70 guided lessons · 69 integrated mastery activities. Visual explanations and purpose-built understanding checks live in the same learning path.</p>${progress(all.done,all.total)}</section><div class="v4-unit-grid">${C.units.map(u=>{const s=unitStats(u.id);return `<article class="v4-unit-card"><span class="v4-unit-card__num">${String(u.id).padStart(2,'0')}</span><h3>${esc(u.title)}</h3><p>${esc(u.scope)}</p>${progress(s.done,s.total,'unit')}<div class="v4-unit-card__meta"><span>${s.total} connected activities</span><button class="v4-btn v4-btn--secondary" data-v4i="unit" data-id="${u.id}">Open</button></div></article>`}).join('')}</div></div>`;bind();}
  function unitPath(s){
    const out=[];let mi=0;
  ...
  function lesson(id){const l=Gd.lessons.find(x=>x.id===id);if(!l)return map();current={type:'lesson',id,unit:l.v4Unit};window.CanonV4Guided.renderLesson(l,host);injectBack(l.v4Unit);}
  function injectBack(unitId){const article=host.firstElementChild;if(!article)return;const b=document.createElement('button');b.className='v4-btn v4-btn--secondary';b.dataset.v4i='unit';b.dataset.id=unitId;b.textContent='← Unit';article.prepend(b);bind();}
  function mastery(id){const m=M[id];if(!m)return map();const u=findUnit(id);current={type:'mastery',id,unit:u};host.innerHTML=`<article class="v4-lesson"><button class="v4-btn v4-btn--secondary" data-v4i="unit" data-id="${u}">← Unit</button><header class="v4-lesson__head"><p class="v4-eyebrow">Applied mastery</p><h2>${esc(m.title)}</h2><p class="v4-objective">${esc(m.dek)}</p></header>${V.render(m.visual)}<div class="v4-prose">${m.body.map(p=>`<p>${esc(p)}</p>`).join('')}</div><aside class="v4-callout" data-tone="plain"><h3>In plain English</h3><p>${esc(m.plain)}</p></aside><button class="v4-btn" data-v4i="mastery-game" data-id="${esc(id)}">Try the understanding check</button></article>`;bind();}
