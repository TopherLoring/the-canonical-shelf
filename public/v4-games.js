...
window.CanonV4Games=(()=>{
  ...
  function init(challenge){
    ...
    if(['sequence','sequence-path','timeline-sort','verse-rebuild'].includes(state.type)){
      ...
      if(JSON.stringify(state.order)===JSON.stringify(c.answer))state.order.push(state.order.shift());
    }
    if(state.type==='match'||state.type==='match-board')state.leftOrder=shuffled((c.items||[]).length);
    return {challenge:c,state};
  }
  const hints=(c,n)=>{const h=Array.isArray(c.hints)?c.hints:[c.hint].filter(Boolean);return h.length?h[Math.min(n,h.length-1)]:'';};
  function progress(current,total){return `<div class="v4-progress"><small>${current}/${total}</small><div class="v4-progress__track"><div class="v4-progress__fill" style="width:${Math.max(0,Math.min(100,current/Math.max(total,1)*100))}%"></div></div><small>challenge</small></div>`;}
  function header(c,meta={}){return `<header class="v4-game__head"><div><p class="v4-eyebrow">${esc(meta.eyebrow||'Understanding check')}</p><h2>${esc(c.title||'Try it')}</h2><p>${esc(c.prompt||'')}</p></div>${meta.current&&meta.total?progress(meta.current,meta.total):''}</header>`;}
  function sequenceBoard(c,s){return `<div class="v4-sort-list">${s.order.map((n,i)=>`<div class="v4-sort-card" data-item="${n}"><span>${esc(c.items[n])}</span><span></span><div class="v4-sort-controls"><button type="button" data-game-action="up" data-i="${i}" aria-label="Move earlier" ${i===0||s.correct?'disabled':''}>↑</button><button type="button" data-game-action="down" data-i="${i}" aria-label="Move later" ${i===s.order.length-1||s.correct?'disabled':''}>↓</button></div></div>`).join('')}</div>`;}
  function matchBoard(c,s){
    const options=c.options||[];
    return `<div class="v4-tile-grid">${s.leftOrder.map(i=>`<section class="v4-flow__node"><b>${esc(c.items[i])}</b><div class="v4-tile-grid" style="margin-top:10px">${options.map((o,j)=>`<button type="button" class="v4-tile" data-game-action="match" data-left="${i}" data-right="${j}" aria-pressed="${String(s.matches[i])===String(j)}" ${s.correct?'disabled':''}>${esc(o)}</button>`).join('')}</div></section>`).join('')}</div>`;
  }
  function evidenceBoard(c,s){
    const lanes=c.lanes||['Supported','Possible','Overreach'];
    const laneOf=i=>s.matches[i]??'';
    return `<div class="v4-evidence-board">${lanes.map((lane,li)=>`<section class="v4-evidence-lane"><h4>${esc(lane)}</h4>${(c.items||[]).map((item,i)=>laneOf(i)===li?`<button class="v4-tile" type="button" data-game-action="unplace" data-i="${i}">${esc(item)}</button>`:'').join('')}</section>`).join('')}</div><h4 class="v4-eyebrow" style="margin-top:18px">Evidence to place</h4><div class="v4-tile-grid">${(c.items||[]).map((item,i)=>laneOf(i)===''?`<div class="v4-flow__node"><b>${esc(item)}</b><div class="v4-game__actions" style="margin-top:10px">${lanes.map((lane,li)=>`<button class="v4-btn v4-btn--secondary" type="button" data-game-action="place" data-i="${i}" data-lane="${li}">${esc(lane)}</button>`).join('')}</div></div>`:'').join('')}</div>`;
  }
  function contextLens(c,s){
    const fields=c.fields||['Speaker','Recipient','Situation','Genre','Purpose'];
    return `<div class="v4-tile-grid">${fields.map((f,fi)=>`<section class="v4-flow__node"><b>${esc(f)}</b><div class="v4-tile-grid" style="margin-top:10px">${(c.options?.[fi]||[]).map((o,oi)=>`<button type="button" class="v4-tile" data-game-action="context" data-field="${fi}" data-choice="${oi}" aria-pressed="${String(s.matches[fi])===String(oi)}">${esc(o)}</button>`).join('')}</div></section>`).join('')}</div>`;
  }
  ...
  function argumentBoard(c,s){
    return `<div class="v4-tile-grid">${c.items.map((x,i)=>`<button type="button" class="v4-tile" data-game-action="node" data-i="${i}" aria-pressed="${s.selected.includes(i)}">${esc(x)}</button>`).join('')}</div><div class="v4-callout" data-tone="plain"><h3>Your connections</h3>${s.links.length?`<ul>${s.links.map(([a,b],i)=>`<li>${esc(c.items[a])} → ${esc(c.items[b])} <button type="button" data-game-action="unlink" data-i="${i}">Remove</button></li>`).join('')}</ul>`:'<p>Select one source node and one supported node, then connect them.</p>'}</div>`;
  }
  function compareBoard(c,s){
    const lanes=c.lanes||['A','Both','B'];
    return `<div class="v4-evidence-board">${lanes.map((lane,li)=>`<section class="v4-evidence-lane"><h4>${esc(lane)}</h4>${c.items.map((item,i)=>s.matches[i]===li?`<button class="v4-tile" type="button" data-game-action="unplace" data-i="${i}">${esc(item)}</button>`:'').join('')}</section>`).join('')}</div><div class="v4-tile-grid" style="margin-top:14px">${c.items.map((item,i)=>s.matches[i]===undefined?`<div class="v4-flow__node"><b>${esc(item)}</b><div class="v4-game__actions" style="margin-top:10px">${lanes.map((lane,li)=>`<button class="v4-btn v4-btn--secondary" data-game-action="place" data-i="${i}" data-lane="${li}">${esc(lane)}</button>`).join('')}</div></div>`:'').join('')}</div>`;
  }
  function bookDetective(c,s){return `<div class="v4-callout" data-tone="plain"><h3>Clues</h3>${(c.clues||[]).map(x=>`<p>${esc(x)}</p>`).join('')}</div><div class="v4-tile-grid">${(c.options||[]).map((o,i)=>`<button type="button" class="v4-tile" data-game-action="single" data-i="${i}" aria-pressed="${s.selected[0]===i}">${esc(o)}</button>`).join('')}</div>`;}
  function board(c,s){
  ...
  function render(session,meta={}){
    ...
    const feedback=s.feedback?`<div class="v4-feedback" data-state="${s.correct?'correct':s.attempt?'wrong':''}" role="status" aria-live="polite">${esc(s.feedback)}</div>`:'';
    const hint=s.hint?`<div class="v4-feedback"><strong>Hint ${s.hint}:</strong> ${esc(hints(c,s.hint-1))}</div>`:'';
    const noCheck=(c.kind||c.type)==='scenario';
    return `<section class="v4-game">${header(c,meta)}<div class="v4-game__board">${board(c,s)}${feedback}${hint}</div><footer class="v4-game__footer"><span class="v4-eyebrow">Untimed · retries are free</span><div class="v4-game__actions">${!noCheck&&!s.correct?'<button class="v4-btn" data-game-action="check">Check my work</button>':''}<button class="v4-btn v4-btn--secondary" data-game-action="hint">${s.hint?'Another hint':'Hint'}</button>${s.correct?'<button class="v4-btn" data-game-action="next">Continue</button>':''}</div></footer></section>`;
  }
  function act(session,action,data={}){
    const {challenge:c,state:s}=session; const type=c.kind||c.type;
    if(action==='up'||action==='down'){const i=+data.i,j=i+(action==='up'?-1:1);if(!s.correct&&j>=0&&j<s.order.length)[s.order[i],s.order[j]]=[s.order[j],s.order[i]];}
    ...
    if(action==='hint')s.hint=Math.min(s.hint+1,Math.max(1,(Array.isArray(c.hints)?c.hints.length:1)));
    if(action==='check'&&!s.correct){s.attempt++;s.correct=evaluate(c,s);s.feedback=s.correct?(c.why||'You built a supported answer.'):'Not yet. Compare your construction with the evidence and revise it; nothing is lost by trying again.';}
    if(action==='choice'&&!s.correct){const st=c.stages[s.stage],i=+data.i;s.feedback=st.feedback?.[i]||'';if(i===st.correct){s.stage++;if(s.stage>=c.stages.length){s.correct=true;s.feedback+=(s.feedback?' ':'')+(c.why||'You completed the scenario.');}}}
    return session;
  }
