...
for(const original of before){const now=D.lessons.find(l=>l.id===original.id);assert.ok(now);const {tracks,skillMissions,...withoutCurriculumMeta}=now;assert.deepEqual(JSON.parse(JSON.stringify(withoutCurriculumMeta)),original,`Baseline lesson changed: ${original.id}`);assert.ok(Array.isArray(tracks)&&tracks.length>0);}
for(const u of D.units){const n=D.lessons.filter(l=>l.unit===u.id).length;assert.ok(n>=4&&n<=5,`Unit ${u.id} has ${n} lessons`);}
assert.deepEqual(Array.from(D.skillTracks,t=>[t.id,t.steps]),[['story',10],['order',17],['groups',12],['chrono',7],['content',8],['themes',7],['verses',8]]);assert.equal(D.skillTracks.reduce((n,t)=>n+t.steps,0),69);assert.equal(D.skillMissions.length,69);assert.equal(new Set(D.skillMissions.map(m=>m.id)).size,69);assert.equal(D.lessons.flatMap(l=>l.skillMissions||[]).length,69);
...
const worker=fs.readFileSync(path.join(root,'worker.js'),'utf8');assert.ok(worker.includes('foundations-skill-source.js')&&worker.includes('foundations-skill-curriculum.js')&&worker.includes('foundations-expansion-finalize.js'));
assert.ok(fs.readFileSync(path.join(pub,'sw.js'),'utf8').includes('canon-v3.7.0'));
console.log('PASS: 70 guided lessons, 69 integrated skill missions, 4–5 lessons/unit, baseline content preservation, corpus coverage, 14 theme investigations, challenge schemas');
